# Mini KITTI Smoke Test

这个脚本用于在不下载完整 KITTI 数据集的情况下，快速验证第五轮的 KITTI 点云读取链路是否能跑通。

它会生成一个很小的 KITTI 风格目录：

data/kitti/
  training/
    velodyne/
      000000.bin
    label_2/
      000000.txt

其中：

velodyne/000000.bin    保存点云，格式是 [x, y, z, intensity]
label_2/000000.txt     保存 KITTI 风格标签

生成 mini sample：

PYTHONPATH=src python scripts/create_mini_kitti_sample.py --frame-id 000000

运行 KITTI 聚类检测 demo：

PYTHONPATH=src python scripts/run_kitti_clustering_demo.py --frame-id 000000

期望看到类似输出：

loaded points: ...
loaded labels: ...
raw detections: ...
detections after nms: ...
saved outputs/figures/kitti_clustering_000000.png

注意：这个 mini sample 是合成点云写成的 KITTI 文件格式，只用于验证文件路径、.bin 解析、label 解析、聚类检测、NMS 和 BEV 可视化流程。

它不是真实 KITTI 数据，也不能作为最终检测指标或 benchmark 结果。

后续如果使用真实 KITTI 数据，需要把真实文件放到：

data/kitti/
  training/
    velodyne/
      000000.bin
    label_2/
      000000.txt